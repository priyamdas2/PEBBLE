...
T_star_Moulton <- function(X,Y,X_0,Beta_hat,SRSWR_index, e_array, G_T_G_inv_G_T)
{n <- dim(X)[1]
p <- dim(X)[2]
e_star_array <- e_array[SRSWR_index]
Beta_star_star <- t(t(Beta_hat))+(1/n)*G_T_G_inv_G_T%*%t(t(e_star_array))

T_star_Moulton_unscaled <- sqrt(n)*(exp(sum(X_0*Beta_star_star)) - exp(sum(X_0*Beta_hat)))

return(T_star_Moulton_unscaled)
}
...
bounds_Moulton <- function(X,Y,X_0,num_BOOT,x0,iter_no)
{n <- dim(X)[1]
p <- dim(X)[2]
...
T_star_Moulton_alt_array <- array(NA,num_BOOT)
...
{ #print(i)
  ...
  T_star_Moulton_alt_array[i] <- T_star_Moulton(X,Y,X_0,Beta_hat,SRSWR_index, e_array, G_T_G_inv_G_T)
}
...
T_star_j_05_alt <- quantile(T_star_Moulton_alt_array, .05)
T_star_j_10_alt <- quantile(T_star_Moulton_alt_array, .10)
T_star_j_90_alt <- quantile(T_star_Moulton_alt_array, .90)
T_star_j_95_alt <- quantile(T_star_Moulton_alt_array, .95)

T_star_quantiles_05_10_90_95 <- c(T_star_j_05_alt,T_star_j_10_alt,T_star_j_90_alt,T_star_j_95_alt)
