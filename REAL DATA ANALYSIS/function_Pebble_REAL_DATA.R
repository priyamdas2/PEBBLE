...
bounds_Pebble <- function(X,Y,b, num_BOOT, beta_a, beta_b,x0,iter_no,Sigma_hat_diag,Norm_var)
{n <- dim(X)[1]
p <- dim(X)[2]
bound_max <- 10
...
####################################################################################
###############  Define bounds_Pebble_alpha ################################################
####################################################################################

bounds_Pebble_alpha <- function(X,Y,b, num_BOOT, beta_a, beta_b,x0,iter_no,Sigma_hat_diag,Norm_var, alpha)
{n <- dim(X)[1]
p <- dim(X)[2]
bound_max <- 10
#### Finding Beta_hat and L_hat ##############
...
H_star_j_alpha_by_2 <- array(NA,p)
H_star_j_alpha <- array(NA,p)
H_star_j_one_minus_alpha <- array(NA,p)
H_star_j_one_minus_alpha_by_2 <- array(NA,p)

H_star_whole_05_10_90_95 <- array(NA,4)

if(length(to_be_selected) >= 20)
{index_q_alpha_by_2 <- array(NA,p)
index_q_alpha <- array(NA,p)
index_q_one_minus_alpha <- array(NA,p)
index_q_one_minus_alpha_by_2 <- array(NA,p)
...
for(j in 1:p)
{index_q_alpha_by_2[j] <- which.min(abs(H_star_MATRIX_SELECTED[,j] - quantile(H_star_MATRIX_SELECTED[,j], .05)))
index_q_alpha[j] <- which.min(abs(H_star_MATRIX_SELECTED[,j] - quantile(H_star_MATRIX_SELECTED[,j], .1)))
index_q_one_minus_alpha[j] <- which.min(abs(H_star_MATRIX_SELECTED[,j] - quantile(H_star_MATRIX_SELECTED[,j], .90)))
index_q_one_minus_alpha_by_2[j] <- which.min(abs(H_star_MATRIX_SELECTED[,j] - quantile(H_star_MATRIX_SELECTED[,j], .95)))

H_star_j_alpha_by_2[j] <- H_star_MATRIX_SELECTED[index_q_05[j],j]
H_star_j_alpha[j] <- H_star_MATRIX_SELECTED[index_q_10[j],j]
H_star_j_one_minus_alpha[j] <- H_star_MATRIX_SELECTED[index_q_90[j],j]
H_star_j_one_minus_alpha_by_2[j] <- H_star_MATRIX_SELECTED[index_q_95[j],j]
}

index_whole <- which.min(abs(H_star_norms_SELECTED - quantile(H_star_norms_SELECTED, alpha/2)))
index_whole[2] <- which.min(abs(H_star_norms_SELECTED - quantile(H_star_norms_SELECTED, alpha)))
index_whole[3] <- which.min(abs(H_star_norms_SELECTED - quantile(H_star_norms_SELECTED, 1-alpha)))
index_whole[4] <- which.min(abs(H_star_norms_SELECTED - quantile(H_star_norms_SELECTED, 1-alpha/2)))

H_star_whole_05_10_90_95 <- H_star_norms_SELECTED[index_whole]
}

return(list(H_star_j_alpha_by_2 = H_star_j_alpha_by_2, H_star_j_alpha = H_star_j_alpha,
            H_star_j_one_minus_alpha = H_star_j_one_minus_alpha,
            H_star_j_one_minus_alpha_by_2 = H_star_j_one_minus_alpha_by_2,
            z_norm_star_record = z_norm_star_record) )
}
